...
def obtener_dataset_ejemplo() -> List[Dict[str, Any]]:
...
def ordenar_por_nombre(lista: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Retorna una copia del listado ordenado ascendentemente por el campo 'name'.
    """
    return sorted(lista, key=lambda x: x["name"])


# 2. Búsqueda de Posición
def buscar_posiciones(lista: List[Dict[str, Any]]) -> None:
    """
    Determina e imprime en qué índice se encuentran "The Thing" y "Rocket Raccoon".
    """
    for idx, personaje in enumerate(lista):
        nombre = personaje["name"]
        if nombre in ["The Thing", "Rocket Raccoon"]:
            print(f"   * '{nombre}' se encuentra en la posicion: {idx}")


# 3. Filtrado de Villanos
def filtrar_villanos(lista: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Retorna la lista de personajes cuyo tipo sea 'is_villain' True.
    """
    return [p for p in lista if p["is_villain"]]


# 4. Cola de Villanos y Filtrado Cronológico
def procesar_cola_villanos(lista: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Encola a todos los villanos y determina cuáles aparecieron antes de 1980.
    """
    villanos = filtrar_villanos(lista)
    # Crear estructura de Cola (FIFO)
    cola = deque(villanos)
    villanos_antiguos = []

    # Procesar la cola elemento por elemento
    while cola:
        personaje = cola.popleft()
        if personaje["first_appearance"] < 1980:
            villanos_antiguos.append(personaje)
            
    return villanos_antiguos


# 5. Filtrado por Iniciales
def filtrar_superheroes_iniciales(lista: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Retorna los superhéroes (is_villain = False) cuyos nombres comiencen con 'Bl', 'G', 'My' o 'W'.
    """
    prefijos = ("Bl", "G", "My", "W")
    return [
        p for p in lista 
...
# 6. Ordenamiento por Nombre Real
def ordenar_por_nombre_real(lista: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Retorna una copia de la lista ordenada ascendentemente por 'real_name'.
    Si 'real_name' es None, se maneja como un string vacío para evitar errores de ordenación.
    """
    return sorted(lista, key=lambda x: x["real_name"] or "")


# 7. Ordenamiento Cronológico de Superhéroes
def ordenar_superheroes_cronologico(lista: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Retorna una copia de los superhéroes ordenada de manera ascendente por 'first_appearance'.
    """
    superheroes = [p for p in lista if not p["is_villain"]]
    return sorted(superheroes, key=lambda x: x["first_appearance"])


# 8. Modificación de Datos
def modificar_nombre_real_ant_man(lista: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Busca a 'Ant Man' y modifica su nombre real a 'Scott Lang'. 
    Retorna el personaje modificado o None si no se encuentra.
    """
    for personaje in lista:
        if personaje["name"].strip().lower() == "ant man":
            personaje["real_name"] = "Scott Lang"
            return personaje
    return None


# 9. Búsqueda en Biografía
def buscar_en_biografia(lista: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Busca y retorna los personajes que contengan la palabra 'time-traveling' o 'suit' en su biografía.
    Es insensible a mayúsculas y minúsculas y requiere límites de palabra.
    """
    import re
    patron = re.compile(r'\b(time-traveling|suit)\b', re.IGNORECASE)
    resultados = []
    for personaje in lista:
        bio = personaje["short_bio"] or ""
        if patron.search(bio):
            resultados.append(personaje)
    return resultados


# 10. Eliminación y Retorno
def eliminar_personajes(lista: List[Dict[str, Any]], nombres_a_eliminar: List[str]) -> None:
    """
    Elimina los personajes indicados por su nombre en la lista original.
    Imprime su información completa antes de borrar o indica si no existían.
    """
    for nombre in nombres_a_eliminar:
        encontrado = None
        for personaje in lista:
            if personaje["name"].strip().lower() == nombre.strip().lower():
                encontrado = personaje
                break
        
        if encontrado:
            print(f"- Eliminando personaje '{nombre}':")
            print(f"  * Nombre: {encontrado['name']}")
            print(f"  * Nombre Real: {encontrado['real_name']}")
            print(f"  * Alias: {encontrado['alias']}")
            print(f"  * Anio de Aparicion: {encontrado['first_appearance']}")
            print(f"  * Biografia: {encontrado['short_bio']}")
            lista.remove(encontrado)
        else:
            print(f"- Personaje '{nombre}' no se encuentra en la lista.")


if __name__ == "__main__":
    marvel_chars = obtener_dataset_ejemplo()
    
    # 1. Ordenamiento por Nombre
    print("1. PERSONAJES ORDENADOS POR NOMBRE (ASCENDENTE):")
    marvel_chars = ordenar_por_nombre(marvel_chars)
    for p in marvel_chars:
        print(f"   * {p['name']}")
    print()

    # 2. Búsqueda de Posición
    print("2. POSICIONES DE THE THING Y ROCKET RACCOON EN LA LISTA ORDENADA:")
    buscar_posiciones(marvel_chars)
    print()

    # 3. Filtrado de Villanos
    print("3. LISTADO DE VILLANOS:")
    villanos = filtrar_villanos(marvel_chars)
    for v in villanos:
        print(f"   * {v['name']}")
    print()

    # 4. Cola de Villanos (procesar < 1980)
    print("4. VILLANOS DE LA COLA QUE APARECIERON ANTES DE 1980:")
    villanos_antes_1980 = procesar_cola_villanos(marvel_chars)
    for v in villanos_antes_1980:
        print(f"   * {v['name']} (Anio: {v['first_appearance']})")
    print()

    # 5. Filtrado por Iniciales ("Bl", "G", "My", "W")
    print("5. SUPERHEROES QUE COMIENZAN CON Bl, G, My o W:")
    iniciales = filtrar_superheroes_iniciales(marvel_chars)
    for p in iniciales:
        print(f"   * {p['name']}")
    print()
    ...
    por_nombre_real = ordenar_por_nombre_real(marvel_chars)
    for p in por_nombre_real:
        n_real = p['real_name'] if p['real_name'] is not None else "N/A"
        print(f"   * {n_real} - {p['name']}")
    print()

    # 7. Ordenamiento Cronológico de Superhéroes
    print("7. SUPERHEROES ORDENADOS POR FECHA DE APARICION:")
    cronologico = ordenar_superheroes_cronologico(marvel_chars)
    for p in cronologico:
        print(f"   * {p['first_appearance']}: {p['name']}")
    print()

    # 8. Modificación de Datos (Ant Man)
    print("8. MODIFICACION DEL NOMBRE REAL DE ANT MAN A SCOTT LANG:")
    modificado = modificar_nombre_real_ant_man(marvel_chars)
    if modificado:
        print(f"   * Nuevo nombre real de Ant Man: {modificado['real_name']}")
    print()

    # 9. Búsqueda en Biografía ("time-traveling" o "suit")
    print("9. PERSONAJES CON LA PALABRA 'time-traveling' O 'suit' EN SU BIOGRAFIA:")
    coincidencias_bio = buscar_en_biografia(marvel_chars)
    for p in coincidencias_bio:
        print(f"   * {p['name']}: \"{p['short_bio']}\"")
    print()

    # 10. Eliminación y Retorno (Electro y Baron Zemo)
    print("10. ELIMINACION DE ELECTRO Y BARON ZEMO:")
    eliminar_personajes(marvel_chars, ["Electro", "Baron Zemo"])
